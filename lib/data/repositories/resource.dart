...
class ResourceRepository {
  ...
  Future<ImageProvider?> getThumbnailImage(String resourceId) async {
    try {
      ...
      if (resource?.thumbnail?.startsWith('file') == true) {
        ...
        /*
        final fname = resource.thumbnail!.split('/').last;
        await _sts.downloadFile(
            Uri.parse(resource.thumbnail!), resourceId, fname, headers);
      ...
      try {
        ...
        for (final item in resource.items) {
          ...
          if (item.uri.startsWith('file')) {
            ...
            if (resource.auth?.method == AuthMethod.sophia) break;
          }
        }
  ...
  Future<Map<String, String>?> _getAuthHeaders(
    ...
    } else if (auth.method == AuthMethod.sophia && serverId != null) {
      // oauth => refresh token
      try {
